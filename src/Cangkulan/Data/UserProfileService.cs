...
namespace Cangkulan.Data
{
    public class UserProfileService : ICrud<UserProfile>
    {
        ...
        public UserProfile GetItemByUsername(string UName)
        {
            ...
            var selItem = db.UserProfiles.Include(c=>c.ProjectBids).Include(c => c.BookmarkedFreelancers).Include(c => c.BookmarkedProjects).Include(c => c.BookmarkedJobs).Include(c=>c.Attachments).Where(x => x.Username.ToLower() == UName.ToLower()).FirstOrDefault();
            return selItem;
        }
