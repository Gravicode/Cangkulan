...
function LoadMaps(datas) {
	...
	if (document.getElementById("map") !== null) {
		...
		for (let i = 0; i < datas.length; i++) {
			var data = datas[i];
			locations.push([locationData(data.pageurl, data.logourl, data.company, data.jobtitle, data.verified), data.lat, data.lon, data.index, data.icon]);
		}
